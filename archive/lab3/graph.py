...
# ============================================================================
# HITL Version: Single Agent + Tools with Human Approval for High-Risk Tool
# ============================================================================

def create_hitl_simple_restorai_graph():
    """
    Create a single-agent ReAct graph with a Human-in-the-Loop (HITL) gate
    before executing the high-risk tool `order_products`.

    How HITL works in this demo:
    - If the agent calls `order_products`, execution pauses and asks the human to:
      - Approve (execute as-is)
      - Edit arguments (paste JSON)
      - Cancel (skip execution)
    - The resulting action (executed/cancelled) is fed back to the agent as a ToolMessage.
    """
    class SimpleState(TypedDict):
        messages: Annotated[Sequence[BaseMessage], operator.add]
...
1. analyze_furniture_image: Analyze uploaded furniture images (skip if no image path provided)
2. search_restoration_knowledge: Search curated restoration guides for techniques and safety info
3. search_web_for_products: Find product availability and price ranges
4. order_products: HIGH-RISK ACTION. Only call this if the user explicitly asks to order/buy items.

WORKFLOW:
1. Use search_restoration_knowledge for techniques and safety
2. Build a shopping list
3. ONLY if the user asked to order/buy: call order_products with items + shipping_address
"""
    ...
    def hitl_tools_node(state: SimpleState):
        ...
        for tc in tool_calls:
            ...
            tool_call_id = tc.get("id", "hitl_tool_call")

            if tool_name == "order_products":
                print("\n" + "=" * 70)
                ...
                if choice == "c":
                    out_messages.append(
                        ToolMessage(
                            content=json.dumps(
                                {"ok": False, "cancelled": True, "reason": "Human cancelled tool execution"},
                                indent=2,
                            ),
                            name=tool_name,
                ...
                if choice == "e":
                    edited = input("Paste edited JSON args (single line or valid JSON): ").strip()
                    try:
                        tool_args = json.loads(edited)
                    except Exception as e:
                        out_messages.append(
                            ToolMessage(
                                content=json.dumps(
                                    {"ok": False, "error": f"Invalid JSON provided by human: {e}"},
                                    indent=2,
                                ),
                                name=tool_name,
                                tool_call_id=tool_call_id,
                ...
                # approve (default if not cancel)
                tool_obj = tool_map.get(tool_name)
                if not tool_obj:
                    out_messages.append(
                        ToolMessage(
                            content=json.dumps({"ok": False, "error": f"Tool not found: {tool_name}"}),
                            name=tool_name,
                            tool_call_id=tool_call_id,
                        )
                    )
                ...
                out_messages.append(ToolMessage(content=str(result), name=tool_name, tool_call_id=tool_call_id))
                continue

            # normal tool execution (no interrupt)
            tool_obj = tool_map.get(tool_name)
            if not tool_obj:
                out_messages.append(
                    ToolMessage(
