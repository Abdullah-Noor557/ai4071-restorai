...
class OrderProductsInput(BaseModel):
    """Input schema for high-risk purchase/order tool (HITL protected)."""
    items: List[str] = Field(
        description="List of products/items to order (human-readable)."
    )
    shipping_address: str = Field(
        description="Shipping address for the order (for demo you can put 'LOCAL_TEST')."
    )
    ...
    notes: Optional[str] = Field(
        default=None,
        description="Optional notes/instructions for the order."
    )
    ...
    try:
        ...
        if not api_key:
            return json.dumps({
                ...
                "note": "This is a demo response. Set GOOGLE_API_KEY for real analysis."
            })
...
# ============================================================================
# Tool 4 (High-Risk Action): Order Products (HITL required)
# ============================================================================
...
def order_products(
    ...
    """
    High-risk action tool: create a local "order" record for the requested items.

    Why this is high-risk:
    - In real systems, ordering products triggers financial transactions and shipping.
    - For the mid-exam, we implement a SAFE but functional execution: it writes an order
      JSON file locally. This still demonstrates the risk category and the need for HITL.

    IMPORTANT:
    - This tool MUST be executed only after human approval in the HITL demo graph.
    - The human may edit items/address/budget/notes before execution.

    Returns:
        JSON string confirming the written order record and file path.
    """
    ...
    filename = f"order_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    order_path = orders_dir / filename
    order_path.write_text(json.dumps(order_payload, indent=2), encoding="utf-8")
...
class KnowledgeBase:
    ...
    def __init__(self):
        if not self._initialized:
            try:
                        # Initialize ChromaDB client (from Lab 2 - in parent directory)
