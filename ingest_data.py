    ...
    def __init__(self, data_directory: str, collection_name: str = "restoration_knowledge"):
        ...
        if chroma_host:
            ...
            self.chroma_client = chromadb.HttpClient(
                host=chroma_host, port=port, settings=settings
            )
        else:
            chroma_path = os.getenv("CHROMA_DB_PATH")
            persist_path = (
                str(Path(chroma_path).resolve())
                if chroma_path
                else "./chroma_db"
            )
...
def main():
    ...
    raw = os.getenv("RESTORATION_GUIDES_DIR", "./data/restoration_guides")
    data_path = Path(raw)
    DATA_DIR = str(
        data_path.resolve()
        if data_path.is_absolute()
        else (repo_root / data_path).resolve()
    )
